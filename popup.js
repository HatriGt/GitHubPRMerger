...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('debug-toggle').addEventListener('click', function() {
      ...
      if (debugDetails.classList.contains('hidden')) {
  ...
  // Toggle between preset and custom branch input
  ...
  function handlePrResults(results) {
    ...
    // Process and display the data in our popup
    document.getElementById('pr-title').textContent = prData.prTitle || 'Unknown Title';
    
    // Convert the HTML description to safe text for preview
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = prData.prDescription || '';
    document.getElementById('pr-description').textContent = tempDiv.textContent.slice(0, 200) + (tempDiv.textContent.length > 200 ? '...' : '');
    
    document.getElementById('source-branch').textContent = prData.sourceBranch || 'Unknown Branch';
    
    // Show debug info if available
    if (prData.debug) {
      const debugDetails = document.getElementById('debug-details');
      debugDetails.innerHTML = '<pre>' + JSON.stringify(prData.debug, null, 2) + '</pre>';
    }
